...
int lm(int argc, char** argv){
    ...
    int* maxj; maxj=(int*)calloc(7, sizeof(int));
    double* maxp; maxp=(double*)calloc(7, sizeof(double));
...
*/
                    ...
                    if(print_posterior>0 && atac>0){// posterior calculation after fitting
                        ...
                        if(atac>0){
                            ...
                            getLeadVar(bf+geta, bf+(j+1)*nbivars+geta, bfmr+(j+1)*nbivars+geta, bfmr2+(j+1)*nbivars+geta, eta0+geta, eta+geta, eta+(j+1)*nbivars+geta, Pi1[0], Pi1[j+1], w+geta, nloci, pp2, pp13, loccatid+geta, maxp, maxj);
                        ...
                        if(outf==NULL){
                            ...
                            if(atac>0){
                                // lead vars
                                printf("\t%s %d %d\t%s %d %d\t%s %d %d\t%s %d %d\t%s %d %d\t%s %d %d\t%s %d %d\n", rss[maxj[0]+geta], loccat[maxj[0]+geta], vt[maxj[0]+geta], 
                                    rss[maxj[1]+geta], loccat[maxj[1]+geta], vt[maxj[1]+geta],
                                    rss[maxj[2]+geta], loccat[maxj[2]+geta], vt[maxj[2]+geta],
                                    rss[maxj[3]+geta], loccat[maxj[3]+geta], vt[maxj[3]+geta],
                                    rss[maxj[4]+geta], loccat[maxj[4]+geta], vt[maxj[4]+geta],
                                    rss[maxj[5]+geta], loccat[maxj[5]+geta], vt[maxj[5]+geta],
                                    rss[maxj[6]+geta], loccat[maxj[6]+geta], vt[maxj[6]+geta]);
                                // effect sizes
                                /*printf("\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n",
                                    be[maxj[0]+geta], be[maxj[1]+geta+(j+1)*nbivars], 
                                    be[maxj[2]+geta], be[maxj[3]+geta+(j+1)*nbivars], 
                                    be[maxj[4]+geta], be[maxj[4]+geta+(j+1)*nbivars],
                                    be[maxj[5]+geta], be[maxj[5]+geta+(j+1)*nbivars],
                                    be[maxj[6]+geta], be[maxj[6]+geta+(j+1)*nbivars],
                                    nk_cor(ds+ (maxj[2]+geta)*samplesize, ds+ (maxj[3]+geta)*samplesize, samplesize));*/
                                //for(k=0; k<7; k++){printf("\t%s", rss[maxj[k]], );}
                            }else{ for(k=0; k<5; k++){printf("\t%lf", pp13[k]);}printf("\n"); }
                        }else{
                            gzprintf(outf, "%d\t%d", fid, fid2+j);
                            if(atac>0){
                                gzprintf(outf,"\t%s %d %d\t%s %d %d\t%s %d %d\t%s %d %d\t%s %d %d\t%s %d %d\t%s %d %d\n", rss[maxj[0]+geta], loccat[maxj[0]+geta], vt[maxj[0]+geta],
                                    rss[maxj[1]+geta], loccat[maxj[1]+geta], vt[maxj[1]+geta],
                                    rss[maxj[2]+geta], loccat[maxj[2]+geta], vt[maxj[2]+geta],
                                    rss[maxj[3]+geta], loccat[maxj[3]+geta], vt[maxj[3]+geta],
                                    rss[maxj[4]+geta], loccat[maxj[4]+geta], vt[maxj[4]+geta],
                                    rss[maxj[5]+geta], loccat[maxj[5]+geta], vt[maxj[5]+geta],
                                    rss[maxj[6]+geta], loccat[maxj[6]+geta], vt[maxj[6]+geta]); 
                                /*gzprintf(outf, "\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n",
                                    be[maxj[0]+geta], be[maxj[1]+geta+(j+1)*nbivars], 
                                    be[maxj[2]+geta], be[maxj[3]+geta+(j+1)*nbivars], 
                                    be[maxj[4]+geta], be[maxj[4]+geta+(j+1)*nbivars],
                                    be[maxj[5]+geta], be[maxj[5]+geta+(j+1)*nbivars],
                                    be[maxj[6]+geta], be[maxj[6]+geta+(j+1)*nbivars],
                                    nk_cor(ds+ (maxj[2]+geta)*samplesize, ds+ (maxj[3]+geta)*samplesize, samplesize));*/
                                //for(k=0; k<10; k++){gzprintf(outf, "\t%lf", log(pp13[k]));} gzprintf(outf, "\n"); 
                            }else{ for(k=0; k<5; k++){printf("\t%lf", pp13[k]);}printf("\n"); }
                            gzclose(outf);
                        }
