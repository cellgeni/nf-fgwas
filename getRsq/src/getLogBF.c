...
// se is estimator of sigma not SE for beta!
double getBetaSigma(double* g, double* y, int n, double* se){
    int i;
    ...
    (*se) = sqrt(vy*(1.-rgy2)/(dn-2.)*dn);
    return vgy/vg;
}

double getMRSE(double* y, double* y2, int n, double mrbe){
    int i;
    double my, my2;
    my = my2 = 0.0;
    double dn = (double)n;
    ...
    double s2 = 0.0;
    for(i=0; i<n; i++){
        s2 += pow((y2[i]-my2)-mrbe*(y[i]-my), 2.0);
    }
    s2 = s2/(dn-2.);
    return sqrt(s2);
}
...
double getLogWABFMR(double* g, double* x, double* y, int n){
    ...
    double lbf = 0.0;
    double r;
    double t2 = ((dn-2.)*gy*gy/(1.-xy*xy+(gy/gx-xy)*(gy/gx-xy)));
    double z2 = ptqn(sqrt(t2), dn); //z2 = z2*z2;
    if(isinf(z2*z2)>0){z2 = t2*t2;}else{z2 = z2*z2;}
    if(z2>100000.){fprintf(stderr, "BF (%lf -> %lf) is too large\n", t2*t2, z2);}
    for(i=0; i<1; i++){
    ...
    if(isinf(lbf)>0 || isnan(lbf)>0){fprintf(stderr, "mrbf is inf: lbf=%lf r=%lf z2=%lf xy=%lf gy=%lf gx=%lf f=%lf\n", lbf, r, z2, xy, gy, gx, sqrt((dn-2.)*gy*gy/(1.-xy*xy+(gy/gx-xy)*(gy/gx-xy)))); lbf=0.0;}
    return lbf;
}
