...
int parseInfo(char* str, char* infostr, VCF_info* vinfo){
    ...
    for(i=0; i<nfield; i++){
        ...
        if(strcmp(infostr,"VT=SNP")==0){
            ...
            vinfo->VT=VT_SV;
        }else if(startWith("INFO=",infostr)==0){
            sscanf(infostr, "INFO=%lf", &(vinfo->RSQ));
...
int getBiVCF(const char* fname, const char* reg, double** pds1, int* psamplesize, int* pnbivars, char** pchr, int** ppos, char*** prss, char*** pba0, char*** pba1, int** vtype){
    getBiVCFRsq(fname, reg,pds1, psamplesize, pnbivars, pchr, ppos, prss, pba0, pba1, vtype, NULL);
}
...
int getBiVCFRsq(const char* fname, const char* reg, double** pds1, int* psamplesize, int* pnbivars, char** pchr, int** ppos, char*** prss, char*** pba0, char*** pba1, int** vtype, double** rsqs){
    ...
    int maxnbivars = 1;
    int ncol_ds1 = nrowVCF(fname, reg, &maxnbivars, &nvars, &nsamples);
    ...
    if(rsqs!=NULL){ (*rsqs) = (double*)calloc(ncol_ds1, sizeof(double)); }
    
    // binarised GL and GT
    double* bgl; bgl = (double*)calloc(nsamples*3*maxnbivars, sizeof(double));
    int*    bgt; bgt = (int*)   calloc(nsamples*2*maxnbivars, sizeof(int));
    ...
    int l=0, indel=0;
    while (tbx_itr_next(fp, tbx, itr, &str) >= 0){
        clearAs0(bgl, nsamples*3*maxnbivars);
        printOrig=0;
        ...
        if(strlen(a0)>1 || strlen(a1)>2*nal-3){indel=1;}else{indel=0;}
        ...
        for(i=0; i<nsamples; i++){
            ...
            //ds[gt[0]]++;
            //ds[gt[1]]++;
            if(doseFormatExist(formatID, nfields, FORMAT_DS)==0){// DS field isn't existing
                if(gt[0]<0){
                    ds[1]=-999999;
                }else{
            ...
            if(nal>2){
                getCombk(nal, 0, work, -1, nbivars1, ds, (*pds1)+nbivars*nsamples+i, nsamples, gt, bgt+i*2, gl, bgl+i*3);
            }else{
                (*pds1)[nbivars*nsamples+i] = ds[1];
                //(*pds1)[nbivars*nsamples+i] = (double)gt[0]+(double)gt[1];
                bgt[i*2+0] = gt[0];
                bgt[i*2+1] = gt[1];
                bgl[i*3+0] = gl[0];
                bgl[i*3+1] = gl[1];
        ...
        for(i=0; i<nbivars1; i++){
            ...
            //printOrig += (vinfo.RSQ>0.3 || vinfo.RSQ<0.0) && af[i]>0.05 && af[i]<0.95 && strlen(a0)==1 && strlen(a1)==1 ? 1 : 0; // maf 0.0
            //printOrig += vinfo.RSQ>0.3 && af[i]>0.15 && af[i]<0.85 && strlen(a0)==1 && strlen(a1)==1 ? 1 : 0; // maf 0.15
            //printOrig += (vinfo.RSQ>0.3 || vinfo.RSQ<0.0) && af[i]>0.10 && af[i]<0.90 && (strlen(a0)==1 && strlen(a1)==1) ? 1 : 0; // maf 0.0 SNP
            //printOrig += (vinfo.RSQ>0.3 || vinfo.RSQ<0.0) && af[i]>0.0 && af[i]<1.0 && (strlen(a0)>1 || strlen(a1)>1) ? 1 : 0; // maf 0.0 non SNP
            //printOrig += (vinfo.RSQ>0.3 || vinfo.RSQ<0.0) && af[i]>0.2 && af[i]<0.8 && strlen(a0)==1 && strlen(a1)==1 ? 1 : 0; // maf 0.2 SNP
            printOrig += (vinfo.RSQ>0.3 || vinfo.RSQ<0.0) && af[i]>0.4 && af[i]<0.6 && strlen(a0)==1 && strlen(a1)==1 ? 1 : 0; // maf 0.4 SNP
        }
        ...
        if((strlen(a0)>1 || strlen(a1)>1)){
            for(j=0; j<nbivars1; j++){
                af[j] =  nk_dsum((*pds1)+(nbivars+j)*nsamples, nsamples, 1)/2.0/(double)nsamples;
                if(vinfo.RSQ>0.3 && af[j]>0.05 && af[j]<0.95){
                    if(indel>0){
                        if(nal>2){
                            printf("%s\t%ld\t%s\t%s\t%s\t%s\t%s\t%s;VT=INDEL;MULTI_ALLELIC\t%s:AS", chr, pos, rs, pba0[0][nbivars+j], pba1[0][nbivars+j], qual, filter, info, format);
                        }else{
                            printf("%s\t%ld\t%s\t%s\t%s\t%s\t%s\t%s;VT=INDEL\t%s:AS", chr, pos, rs, pba0[0][nbivars+j], pba1[0][nbivars+j], qual, filter, info, format);
                        }
                    }else{
                        if(nal>2){
                            printf("%s\t%ld\t%s\t%s\t%s\t%s\t%s\t%s;VT=SNP;MULTI_ALLELIC\t%s:AS", chr, pos, rs, pba0[0][nbivars+j], pba1[0][nbivars+j], qual, filter, info, format);
                        }else{
                            printf("%s\t%ld\t%s\t%s\t%s\t%s\t%s\t%s;VT=SNP\t%s:AS", chr, pos, rs, pba0[0][nbivars+j], pba1[0][nbivars+j], qual, filter, info, format);
                        }
                    }
                    
                    for(i=0; i<nsamples; i++){
                        printf("\t%d|%d:%.2lf:%.2lf,%.2lf,%.2lf:0,0", bgt[j*nsamples*2+i*2+0], bgt[j*nsamples*2+i*2+1], 
                               pds1[0][(nbivars+j)*nsamples+i], bgl[j*nsamples*3+i*3], bgl[j*nsamples*3+i*3+1], bgl[j*nsamples*3+i*3+2]);
                    }
                    printf("\n");
                }
            }
...
int getDose(const char* fname, const char* reg, const int header){ 
    double* ds;
    double* rsqs;
    int samplesize;
    int nbivars;
    ...
    int i, j;
    char sep;
    if(header==1){getVCFHeader(fname);}else if(header==2){getVCFHeaderAll(fname);}
    getBiVCFRsq(fname, reg, &ds, &samplesize, &nbivars, &chr, &pos, &rss, &ba0, &ba1, &vtype, &rsqs);
    for(j=0; j<nbivars; j++){
        double af = nk_dsum(ds+j*samplesize, samplesize, 1)/2.0/(double)samplesize;
        //if(af>0.05&&af<0.95){
            //printf("%s:%s:%s\t", rss[j], ba0[j], ba1[j]);
            //printf("%s:%d:%s:%s\t", chr, pos[j], ba0[j], ba1[j]);
            printf("%s\t%d\t%s\t%s\t%s\t.\t.\t%lf\tDS\t", chr, pos[j], rss[j], ba0[j], ba1[j], rsqs[j]);
            for(i=0; i<samplesize; i++){
...
//for(i=0;i<10;i++){fprintf(stderr, "%lf ", ds1[i]);}
//fprintf(stderr, "\n"); 
//for(i=0;i<139;i++){fprintf(stderr, "%lf ", ds2[i]);} 
//fprintf(stderr, "\n");
    double ldsc_val=0.0;
    double ldsc_val1=0.0;
    double af1 = 0.0;
    double af2 = 0.0;
    if(ldsc>0){
       double* R; R = (double*)calloc(nbivars1*nbivars1, sizeof(double));
       nk_Cor(ds1, R, samplesize, nbivars1);
       for(j=0; j<nbivars1; j++){
           printf("%s\t%d\t%s\t%s\t%s\t", chr, pos1[j], rss1[j], ba0_1[j], ba1_1[j]);
           ldsc_val = 0.0;
           for(i=0; i<nbivars2; i++){
               /*af1 = nk_dsum(ds1+samplesize*j, samplesize, 1)/2.0/(double)samplesize;
               af2 = nk_dsum(ds2+samplesize*i, samplesize, 1)/2.0/(double)samplesize;
               ldsc_val1 = pow(nk_cor(ds1+samplesize*j, ds2+samplesize*i, samplesize), 2.0); 
               if(af1>0.0 && af1<1.0 && af2>0.0 && af2<1.0) ldsc_val += (ldsc_val1-(1.0-ldsc_val1)/((double)samplesize-2.0));*/
               ldsc_val1 = pow(R[i+j*nbivars1], 2.0);
               if(isnan(ldsc_val1)==0)ldsc_val += (ldsc_val1-(1.0-ldsc_val1)/((double)samplesize-2.0)); 
           }
           printf("%lf\n", ldsc_val);
       }
    }else{for(i=0; i<nbivars2; i++){
        for(j=0; j<nbivars1; j++){
            //printf("%s\t%s\t%s\t%s\t%s\t%s\t%s\t", fid, rss2[i], ba0_2[i], ba1_2[i], rss1[j], ba0_1[j], ba1_1[j]);
            printf("%s\t%d\t%s\t%s\t%d\t%s\t%s\t", fid, pos2[i], ba0_2[i], ba1_2[i], pos1[j], ba0_1[j], ba1_1[j]);
        ...
        for(i=0; i<nbivars2; i++){
            for(j=0; j<nbivars1-1; j++){
                for(k=j+1; k<nbivars1; k++){
                    ...
                    printf("%s\t%d\t%s\t%s\t%d;%d\t%s;%s\t%s;%s\t", fid, pos2[i], ba0_2[i], ba1_2[i], pos1[j], pos1[k], ba0_1[j], ba0_1[k], ba1_1[j], ba1_1[k]);
...
int lm(int argc, char** argv){
    ...
    // 
    double Pia = -1.0;
    for(i=0; i<argc-1; i++){if(strcmp(argv[i], "--Pia")==0){ Pia = (double)atof(argv[i+1]); break; } }
 
    for(i=0; i<argc; i++){if(strcmp(argv[i],"--force-print")==0){printBF=3;}}

    int force_fit=0;
    for(i=0; i<argc; i++){if(strcmp(argv[i],"--force-fit")==0){force_fit=1;}}
    ...
    if(npeaks==0 && force_fit==0){fprintf(stderr, "no peak found\n"); return 0;}
    ...
    getBiVCFRsq(fname, reg, &ds, &samplesize, &nbivars, &chr, &pos, &rss, &ba0, &ba1, &vt, &rsq_vcf);
	...
	if(verbose>0) fprintf(stderr, "Pi2=%lf\n", Pi1[1]);
    }
    ...
    for(j=0; j<nbivars; j++){
        ...
        //rsq[j] = rsq_vcf[j]/7.*3.; //nk_var(ds+j*samplesize, ds+j*samplesize, samplesize)/af[j]/(1.0-af[j])/2.0;
        //rsq[j] = rsq_vcf[j]; //nk_var(ds+j*samplesize, ds+j*samplesize, samplesize)/af[j]/(1.0-af[j])/2.0;
        rsq[j] = 1.0; //nk_var(ds+j*samplesize, ds+j*samplesize, samplesize)/af[j]/(1.0-af[j])/2.0;
        ...
        if(af[j]>maf0 && af[j]<1.0-maf0 && rsq[j]>0.3){
            ...
            for(k=0; k<npeaks; k++){// annnotation
                if(pos1bed[k]<=pos[j] && pos[j]<=pos2bed[k]){// in the kth peak
                    //fprintf(stderr,"+");
                    //double covAtj;
                    if(fcoverage!=NULL){ covs[j] = getCovFromBedG(fcoverage, chrom, pos[j]); }else{covs[j]=1.0;} //fprintf(stderr, "%lf\n", covs[j]); }
...
fprintf(stderr, "atac - eqtl\n");
    ...
    //for(j=0; j<nbivars; j++){for(l=0; l<M+1; l++){if(isnan(eta[j+(l)*nbivars])>0){fprintf(stderr, "j=%d l=%d vt=%d cov=%lf eta=%lf\n", j, l, vt[j], covs[j]/ph[k], cov2eta(covs[j]/ph[k], beta2[3], beta2+5, xk, 4, 0));}}}
    ...
    for(i=0; i<argc-1; i++){if(strcmp(argv[i], "--bf2")==0){
        ...
        FILE* fPis; fPis=fopen("/nfs/users/nfs_n/nk5/s117/ATAC100/QTL/Lm/GWAS/Marginal/Pis.bin","rb");
        double* Pis; Pis=(double*)calloc(10, sizeof(double));
        fread(Pis, 10, sizeof(double), fPis);
        int ll = -1;
        while(fscanf(fbf2list, "%s\n", fbf2)!=EOF){
            ...
            int pfbf2flag=0;
            char* pfbf2; for(k=strlen(fbf2)-1; k>=0; k--){if(fbf2[k]=='/'){pfbf2 = fbf2+k+1; pfbf2flag++; if(pfbf2flag==2){ break;}}}
            ...
            if(nrowbf2<10){
                ...
                // Param 5
                colocWithGWAS(bf, bf2, eta0, Pi1[0], Pis[ll], w, nbivars, pp12);
                //pwhmNewAtacGwas(bf, bf2, Pi1[0], Pis[ll], w, nbivars, pp12);
                //printf("%d", fid);
                ...
                if(outf==NULL){
                    ...
                    for(k=0; k<5; k++){gzprintf(outf, "%lf ", log(pp12[k]));} gzprintf(outf, "%lf\n", log(pp12[5])); 
                    gzclose(outf);
                }
                free(posbf2); free(bf2orig); free(bf2); free(pos2bf2); free(eta0);
            }
        }
        return 0;
    }}
                    ...
                    if(print_posterior>0 && atac>0){// posterior calculation after fitting
                        ...
                        if(atac>0){
                            ...
                            if(Pia<0.0){
                                ...
                                pwhmnewWithPia(bf+geta, bf+(j+1)*nbivars+geta, bfmr+(j+1)*nbivars+geta, bfmr2+(j+1)*nbivars+geta, eta0+geta, eta+geta, eta+(j+1)*nbivars+geta, Pi1[0], Pi1[j+1], Pia, w+geta, nloci, pp2, pp13, loccatid+geta);

/*if(fid2+j==496){
int ii;
for(ii=0;ii<nloci;ii++){printf("%lf %lf %lf %lf %lf %lf %lf\n", bf[ii+geta], bf[(j+1)*nbivars+geta+ii], bfmr[(j+1)*nbivars+geta+ii], bfmr2[(j+1)*nbivars+geta+ii], eta0[geta+ii], eta[geta+ii], eta[(j+1)*nbivars+geta+ii]);}
}*/
                        ...
                        if(outf==NULL){
                            ...
                            if(atac>0){ for(k=0; k<9; k++){printf("%lf ", log(pp13[k]));} printf("%lf\n", log(pp13[k])); }else{ for(k=0; k<5; k++){printf("\t%lf", pp13[k]);}printf("\n"); }
                        }else{
                            gzprintf(outf, "%d\t%d\t%lf\t", fid, fid2+j, fabs(midp[fid_bed]-midp[fid2_bed+j])/500000.0);
                            if(atac>0){ for(k=0; k<9; k++){gzprintf(outf, "%lf ", log(pp13[k]));} gzprintf(outf, "%lf\n", log(pp13[k])); }else{ for(k=0; k<5; k++){printf("\t%lf", pp13[k]);}printf("\n"); }
                            gzclose(outf);
                        }
...
int main(int argc, char** argv){
    ...
    if(argc>2){
        ...
        getRsq(argv[1], argv[2], argv[3], NULL, 0);
    }
#elif BAYESLM
    if(verbose_loadVCF>0)fprintf(stderr, "\n\nbayesLm\n\n");
    lm(argc, argv);
#elif GETDOSE
    getDose(argv[1], argv[2], header);
#elif PRINTVCF
