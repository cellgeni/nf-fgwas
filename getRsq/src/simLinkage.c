...
*/
                    ...
                    if(print_posterior>0 && atac>0){// posterior calculation after fitting
                        ...
                        //fprintf(stderr, "%d, %s, %d, %d\t", fid2+j, rss[cvs[0]+geta], loccat[cvs[0]+geta], loccat[(j+1)*nbivars+geta+cvs[0]]);
			            //fprintf(stderr, "%d, %s, %d, %d %lf %lf\t", fid2+j, rss[cvs[1]+geta], loccat[cvs[1]+geta], loccat[(j+1)*nbivars+geta+cvs[1]], be[cvs[1]+geta], se[cvs[1]+geta]);
                        //fprintf(stderr, "%d, %s, %d, %d, %lf %lf %lf\n", fid2+j, rss[cvs[2]+geta], loccat[cvs[2]+geta], loccat[(j+1)*nbivars+geta+cvs[2]], be[cvs[2]+geta], se[cvs[2]+geta], nk_cor(ds+ (cvs[1]+geta)*samplesize, ds+ (cvs[2]+geta)*samplesize, samplesize));
                        
                        double mrbe, mrse;
                        if(hypo==0){// NULL
                            //fprintf(stderr,"null\n");
                            rnorm(ds0, samplesize, 0.0, 0.0, se[cvs[1]+geta],               y_r);
                            rnorm(ds0, samplesize, 0.0, 0.0, se[cvs[2]+geta+(j+1)*nbivars], y2_r);
                        }else if(hypo==1){// single
                            //fprintf(stderr, "single\n");
                            rnorm(ds+(cvs[1]+geta)*samplesize, samplesize, 0.0, be[cvs[1]+geta], se[cvs[1]+geta],               y_r);
                            rnorm(ds0,                         samplesize, 0.0, 0.0,             se[cvs[2]+geta+(j+1)*nbivars], y2_r);
                        }else if(hypo==6){// single
                            rnorm(ds0,                         samplesize, 0.0, 0.0,                           se[cvs[1]+geta],               y_r);
                            rnorm(ds+(cvs[2]+geta)*samplesize, samplesize, 0.0, be[cvs[2]+geta+(j+1)*nbivars], se[cvs[2]+geta+(j+1)*nbivars], y2_r);
                        }else if(hypo==2){// Linkage
                            //fprintf(stderr, "linkage\n");
                            rnorm(ds+(cvs[1]+geta)*samplesize, samplesize, 0.0, be[cvs[1]+geta],               se[cvs[1]+geta],               y_r);
                            rnorm(ds+(cvs[2]+geta)*samplesize, samplesize, 0.0, be[cvs[2]+geta+(j+1)*nbivars], se[cvs[2]+geta+(j+1)*nbivars], y2_r);
                            rsqOI[0] = nk_cor(ds+ (cvs[1]+geta)*samplesize, ds+ (cvs[2]+geta)*samplesize, samplesize);
                        }else if(hypo==3){// Pleio
                        ...
                        }else if(hypo==4){// causal y -> y2
                            
                            mrbe=be[cvs[1]+(j+1)*nbivars+geta]/be[cvs[1]+geta];
                            mrse=getMRSE(y, y2+j*samplesize, samplesize, mrbe);
                            rnorm(ds+(cvs[1]+geta)*samplesize, samplesize, 0.0, be[cvs[1]+geta], se[cvs[1]+geta], y_r);
                            rnorm(y_r,                         samplesize, 0.0, mrbe,            mrse,            y2_r);
                            /*
                            double mrbe=be[cvs[2]+geta]/be[cvs[2]+(j+1)*nbivars+geta];
                            double mrse=getMRSE(y2+j*samplesize, y, samplesize, mrbe);
                        ...
                        }else if(hypo==5){// real pleio
                            int anchor=(int)floor(runif()*((double)M-2.0));
                            if(anchor+fid2>=fid){ anchor++; }
                            if(anchor>=j){ anchor++; }
                            if(anchor>=M){ anchor=M-1;}
                            rcausal1(eta+(anchor+1)*nbivars+geta, eta+geta, eta+(j+1)*nbivars+geta, w+geta, nloci, cvs);
                        
                            //fprintf(stderr, "%d %d %d %d\n", fid, fid2+j, fid2+anchor, loccat[geta+cvs[0]+(anchor+1)*nbivars]);
                            
                            double mrbe = be[cvs[0]              +geta]/be[cvs[0]+(anchor+1)*nbivars+geta];
                            double mrbe2= be[cvs[0]+(j+1)*nbivars+geta]/be[cvs[0]+(anchor+1)*nbivars+geta];
                            double mrse = getMRSE(y2+samplesize*anchor, y,               samplesize, mrbe);
                            double mrse2= getMRSE(y2+samplesize*anchor, y2+j*samplesize, samplesize, mrbe2);
                            rnorm(ds+(cvs[0]+geta)*samplesize, samplesize, 0.0, be[cvs[0]+(anchor+1)*nbivars+geta], se[cvs[0]+(anchor+1)*nbivars+geta], ya_r);
                            //rnorm(ds+(cvs[0]+geta)*samplesize, samplesize, 0.0, 2.0, 0.1, ya_r);
                            rnorm(ya_r, samplesize, 0.0, mrbe,  mrse,  y_r);
                            rnorm(ya_r, samplesize, 0.0, mrbe2, mrse2, y2_r);
                        }else if(hypo==20){// Linkage 2 var
                            //fprintf(stderr, "%d %d %d %d %d %d ", fid, fid2+j, loccat[geta+cvs[2]], loccat[geta+cvs[3]], loccat[geta+cvs[4]+(j+1)*nbivars], loccat[geta+cvs[5]+(j+1)*nbivars]);
                            //fprintf(stderr, "%s %s %s %s\n", rss[geta+cvs[2]], rss[geta+cvs[3]], rss[geta+cvs[4]], rss[geta+cvs[5]]);
                            
                            double se1 = sqrt((pow(se[cvs[2]+geta],              2.)+pow(se[cvs[3]+geta],              2.))/2.);
                            double se2 = sqrt((pow(se[cvs[4]+geta+(j+1)*nbivars],2.)+pow(se[cvs[5]+geta+(j+1)*nbivars],2.))/2.);
                            
                            rnorm2(ds+(cvs[2]+geta)*samplesize, ds+(cvs[3]+geta)*samplesize, samplesize, 0.0, be[cvs[2]+geta],               be[cvs[3]+geta],               se1, y_r);
                            rnorm2(ds+(cvs[4]+geta)*samplesize, ds+(cvs[5]+geta)*samplesize, samplesize, 0.0, be[cvs[4]+geta+(j+1)*nbivars], be[cvs[5]+geta+(j+1)*nbivars], se2, y2_r);
                        }else if(hypo==30){// pleio y <-> y2

                            double se1 = sqrt((pow(se[cvs[0]+geta],              2.)+pow(se[cvs[1]+geta],              2.))/2.);
                            double se2 = sqrt((pow(se[cvs[0]+geta+(j+1)*nbivars],2.)+pow(se[cvs[1]+geta+(j+1)*nbivars],2.))/2.);
                            rnorm2(ds+(cvs[0]+geta)*samplesize, ds+(cvs[1]+geta)*samplesize, samplesize, 0.0, be[cvs[0]+geta],               be[cvs[1]+geta],               se1, y_r);
                            rnorm2(ds+(cvs[0]+geta)*samplesize, ds+(cvs[1]+geta)*samplesize, samplesize, 0.0, be[cvs[0]+geta+(j+1)*nbivars], be[cvs[1]+geta+(j+1)*nbivars], se2, y2_r);
                            rsqOI[0] = nk_cor(ds+ (cvs[0]+geta)*samplesize, ds+ (cvs[1]+geta)*samplesize, samplesize);
                            
                        }else if(hypo==40){// causal y -> y2
                            
                            double se1 = sqrt((pow(se[cvs[2]+geta], 2.)+pow(se[cvs[3]+geta], 2.))/2.);
                            double mrbe=(be[cvs[2]+(j+1)*nbivars+geta]/be[cvs[2]+geta]+be[cvs[3]+(j+1)*nbivars+geta]/be[cvs[3]+geta])/2.0;
                            double mrse=getMRSE(y, y2+j*samplesize, samplesize, mrbe);
                            rnorm2(ds+(cvs[2]+geta)*samplesize, ds+(cvs[3]+geta)*samplesize, samplesize, 0.0, be[cvs[2]+geta], be[cvs[3]+geta], se1, y_r);
                            rnorm(y_r,                         samplesize, 0.0, mrbe,            mrse,            y2_r);
                            rsqOI[0] = nk_cor(ds+ (cvs[2]+geta)*samplesize, ds+ (cvs[3]+geta)*samplesize, samplesize);
                            
                        }else if(hypo==50){// causal y2 -> y

                            double se1 = sqrt((pow(se[cvs[4]+geta+(j+1)*nbivars],2.)+pow(se[cvs[5]+geta+(j+1)*nbivars],2.))/2.);
                            double mrbe=(be[cvs[4]+geta]/be[cvs[4]+(j+1)*nbivars+geta]+be[cvs[5]+geta]/be[cvs[5]+(j+1)*nbivars+geta])/2.0;
                            double mrse=getMRSE(y2+j*samplesize, y, samplesize, mrbe);
                            rnorm2(ds+(cvs[4]+geta)*samplesize, ds+(cvs[5]+geta)*samplesize, samplesize, 0.0, be[cvs[4]+(j+1)*nbivars+geta], be[cvs[5]+(j+1)*nbivars+geta], se1, y2_r);
                            rnorm(y2_r,                         samplesize, 0.0, mrbe,            mrse,            y_r);
                            rsqOI[0] = nk_cor(ds+ (cvs[4]+geta)*samplesize, ds+ (cvs[5]+geta)*samplesize, samplesize);
                            
                        }else if(hypo==300){// L & P
                            
                            double se2 = sqrt((pow(se[cvs[2]+geta+(j+1)*nbivars],2.)+pow(se[cvs[4]+geta+(j+1)*nbivars],2.))/2.);
                            rnorm( ds+(cvs[2]+geta)*samplesize,                              samplesize, 0.0, be[cvs[2]              +geta],                                se[cvs[2]+geta], y_r);
                            rnorm2(ds+(cvs[2]+geta)*samplesize, ds+(cvs[4]+geta)*samplesize, samplesize, 0.0, be[cvs[2]+geta+(j+1)*nbivars], be[cvs[4]+geta+(j+1)*nbivars], se2,             y2_r);
                            rsqOI[0] = nk_cor(ds+ (cvs[2]+geta)*samplesize, ds+ (cvs[4]+geta)*samplesize, samplesize);
                            
                        }else if(hypo==400){// L & C
                            
                            double mrbe=be[cvs[2]+(j+1)*nbivars+geta]/be[cvs[2]+geta];
                            double mrse=getMRSE(y, y2+j*samplesize, samplesize, mrbe);
                            double se2 = sqrt((mrse*mrse + pow(se[cvs[4]+geta+(j+1)*nbivars],2.))/2.);
                            rnorm(ds+(cvs[2]+geta)*samplesize,       samplesize, 0.0, be[cvs[2]+geta],                     se[cvs[2]+geta], y_r);
                            rnorm2(y_r, ds+(cvs[4]+geta)*samplesize, samplesize, 0.0, mrbe, be[cvs[4]+geta+(j+1)*nbivars], se2,             y2_r);
                            rsqOI[0] = nk_cor(ds+ (cvs[2]+geta)*samplesize, ds+ (cvs[4]+geta)*samplesize, samplesize);
                            
                        }else if(hypo==500){// P & C
                            
                            double mrbe=be[cvs[2]+(j+1)*nbivars+geta]/be[cvs[2]+geta];
                            double mrse=getMRSE(y, y2+j*samplesize, samplesize, mrbe);
                            double se2 = sqrt((mrse*mrse + pow(se[cvs[2]+geta+(j+1)*nbivars],2.))/2.);
                            rnorm(ds+(cvs[2]+geta)*samplesize,       samplesize, 0.0, be[cvs[2]+geta],                     se[cvs[2]+geta], y_r);
                            rnorm2(y_r, ds+(cvs[2]+geta)*samplesize, samplesize, 0.0, mrbe, be[cvs[2]+geta+(j+1)*nbivars], se2,             y2_r);
                        ...
                        // calculating BFs
                        for(l=0; l<nbivars; l++){if(af[l]>maf0 && af[l]<1.0-maf0 && rsq[l]>0.3){
                            bf[l]         = getLogWABF(ds+l*samplesize, y_r,  samplesize);
                            bf[l+nbivars] = getLogWABF(ds+l*samplesize, y2_r, samplesize);
                            bfmr[l]     = getLogWABFMR(ds+l*samplesize, y_r, y2_r, samplesize);
                            bfmr2[l]    = getLogWABFMR(ds+l*samplesize, y2_r, y_r, samplesize);
                        }}
                        
                        // calculateing posterior
                        //getIntrPost(bf+geta, bf+(j+1)*nbivars+geta, bfmr+(j+1)*nbivars+geta, bfmr2+(j+1)*nbivars+geta, eta0+geta, eta+geta, eta+(j+1)*nbivars+geta, Pi1[0], Pi1[j+1], Psi, w+geta, nloci, pp2, pp13, loccatid+geta);
                        getIntrPost(bf+geta, bf+nbivars+geta, bfmr+geta, bfmr2+geta, eta0+geta, eta+geta, eta+(j+1)*nbivars+geta, Pi1[0], Pi1[j+1], Psi, w+geta, nloci, pp2, pp13, loccatid+geta);
                        
                        gzFile outf=NULL;
                        for(k=0; k<argc-1; k++){if(strcmp(argv[k],"--output")==0){outf = gzopen(argv[k+1], "ab6f");}}
                        gzprintf(outf, "%d\t%d\t%lf", fid, fid2+j, rsqOI[0]);
                        for(k=0; k<10; k++){gzprintf(outf, "\t%lf", log(pp13[k]));}
 
gzprintf(outf, "\t%d %d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf", loccat[cvs[1]+geta], loccat[(j+1)*nbivars+geta+cvs[1]],
be[cvs[1]+geta], se[cvs[1]+geta],  be[cvs[1]+(j+1)*nbivars+geta], se[cvs[1]+(j+1)*nbivars+geta], mrbe, mrse, 
bf[cvs[1]+geta], bfmr[cvs[1]+geta], bf[cvs[1]+geta+nbivars], bfmr2[cvs[1]+geta]);
