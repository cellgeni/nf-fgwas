...
int lm(int argc, char** argv){
    ...
    double* ds;
    int nbivars, samplesize, downsample;
    char* chr;
    int* pos;
    char** ba0;
    char** ba1;
    ...
    for(i=0; i<argc-1; i++){if(strcmp(argv[i], "--downsample")==0){downsample = atoi(argv[i+1]);}}
    ...
    for(j=0; j<nbivars; j++){
        af[j]  = nk_dsum(ds+j*samplesize, downsample, 1)/2.0/(double)downsample;
        rsq[j] = nk_var(ds+j*samplesize, ds+j*samplesize, downsample)/af[j]/(1.0-af[j])/2.0;
        if(beta!=NULL){
        ...
        if(af[j]>maf0 && af[j]<1.0-maf0 && rsq[j]>0.3){
            ...
            bf[j] = getLogWABF(ds+j*samplesize, y, downsample);
            be[j] = getBeta(ds+j*samplesize, y, downsample, se+j);
            if(fid2>0){// for pairwise
                af2[j]  = nk_dsum(ds2+j*samplesize2,       downsample, 1)/2.0/(double)downsample;
                rsq2[j] = nk_var( ds2+j*samplesize2, ds2+j*samplesize2, downsample)/af2[j]/(1.0-af2[j])/2.0;
                if(af2[j]>maf0 && af2[j]<1.0-maf0 && rsq2[j]>0.3 && af[j]>maf0 && af[j]<1.0-maf0 && rsq[j]>0.3){
                    for(k=0; k<M; k++){
                        ...
                        bf[j+(k+1)*nbivars]    = getLogWABF(  ds2+j*samplesize2,    y2+k*samplesize2, downsample);
                        if(ds==ds2 || samplesize==samplesize2){
                            bfmr[j+(k+1)*nbivars]  = getLogWABFMR(ds2+j*samplesize2, y, y2+k*samplesize2, downsample);
                            bfmr2[j+(k+1)*nbivars] = getLogWABFMR(ds2+j*samplesize2, y2+k*samplesize2, y, downsample);
                        }else{
                        ...
                        be[j+(k+1)*nbivars]    = getBeta(ds2+j*samplesize2, y2+k*samplesize2, downsample, se+(j+(k+1)*nbivars));
                    }
                }else{
                    w[j] = 0.0;
                }
...
int kk;
for(kk=0; kk<npeaks; kk++){if(pos1bed[kk]<=pos[j] && pos[j]<=pos2bed[kk]){break;}}
                    gzprintf(outBFf, "%d\t%lf\t", loccat[j+k*nbivars], (npeaks==kk ? -1.0 : covs[j]/ph[kk]));
